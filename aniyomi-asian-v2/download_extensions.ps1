...
foreach ($extName in $targetExtensions) {
    ...
    if ($ext) {
        ...
        try {
            ...
            # Ensure 'apk' matches our renamed file
            if ($newEntry | Get-Member -Name "apk") {
                $newEntry.apk = "$($extName.ToLower()).apk"
            } else {
                $newEntry | Add-Member -MemberType NoteProperty -Name "apk" -Value "$($extName.ToLower()).apk" -Force
            }
...
$jsonOutput | Out-File "index.min.json" -Encoding UTF8

Write-Host "Start-Process completed. Index generated with $($newIndex.Count) extensions."
